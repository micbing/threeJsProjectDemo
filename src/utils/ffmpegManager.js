...
class FFManager {
     ffmpeg = {}; // ffmpeg实例
     runTask = reactive([]);
     running = ref(false); // 运行状态
     isLoaded = ref(false); // 是否已加载
     showLog = true; // 是否打印输出
     ...
     static Hooks = {
        beforeInit: (ins) => {}, // init之前
        afterInit: (ins) => {} // init之后
    };
    constructor(options) {
        Object.assign(FFManager.Hooks, options?.Hooks || {});
        const createOptions = {
            corePath: '/ffmpeg/ffmpeg-core.js',
            log:false,// this.showLog,
    ...
    // FS写文件
    async writeFile(filePath, fileName, fileUrl, force = false) {
    ...
    // 获取文件buffer
    getFileBuffer(filePath, fileName, format) {
        const localPath = `${fileName}.${format}`;
        return this.ffmpeg.FS('readFile', `${filePath}${localPath}`);
    }
    // 获取文件Blob
    getFileBlob(filePath, fileName, format) {
        const fileBuffer = this.getFileBuffer(filePath, fileName, format);
        return new Blob([fileBuffer], { type: FileTypeMap[format] });
    }
    /**
     * 获取文件url，用于下载
     * */
    getFileUrl(filePath, fileName, format) {
     ...
     * 获取文件file对象
     * */
    getFile(filePath, fileName, format) {
        const localPath = `${fileName}.${format}`;
        const fileBuffer = this.getFileBuffer(filePath, fileName, format);
        return new File(fileBuffer, localPath, { type: FileTypeMap[format] });
    }
    // 创建目录
    mkdir(paths) {
    ...
    // 音频合成
    async mergeAudio(start, trackList, trackAttrMap, fileName, filePath) {
     ...
     * 从视频中分离音频
     * */
    async splitAudio(videoName, format, force = false) {
    ...
    // 生成音波
    async genWave(sourceName, frameCount, format = 'aac', force = false) {
     ...
     * 视频/gif 抽帧（全量，用于轨道）
     * */
    async genFrame(fileName, format, size) {
     ...
     * 视频抽帧指定时间（用于播放）
     * */
    async genPlayFrame(videoName, format, size, time) {
    ...
    // 获取Gif图片
    getGifFrame(gifName, frameIndex) {
    ...
    // 获取指定的某一帧
    async getPIFrame(videoName, format, size, start) {
        const fileName = `/pic`;
        const framePath = `${this.pathConfig.playFrame}${videoName}`;
        const videoFilePath = `${videoName}.${format}`;
